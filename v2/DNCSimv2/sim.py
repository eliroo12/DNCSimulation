...
class sim:
        #stat table build out = WD,Wepdelay,Dex,Critrate,Critdamage,Directrate,Det,skillspeed,gcd
                    #int,     int,  table, table, table,   dict,   dict,    dict,   bool,    bool
    def __init__(self,number,length, open, fight, stats, abilities, party, pbuffs, buffs, log, potion):
    ...
    def sim(self):
        ...
        while self.clock < self.length:
            ...
            #            i.dropbuff(clock)
            # Run through buff tables and apply buffs / drop off and build self.potency table
            ...
            for i in self.party.values():
                if i.nextgcd == self.clock:
                    ...
                    if self.buffs['Technical Finish'].getactive(self.clock) and not delay:
            ...
            if stillinopener:
                ...
                if nextgcd == self.clock:
                    ...
                    if technicaldancing or standarddancing:
                        ...
                        abilityused = True
                        saberhold = False
                    elif self.abilities['Saber Dance'].available(self.clock) and round(nextgcd - 1,2) <= self.clock and self.abilities['Technical Step'].nextuse <= nextgcd:
                        if self.createlog:
                            logging.info(str(self.clock) + " : You use Saber Dance!")
                        self.buffs['Saber Dance'].activate(self.clock)
                        self.schedule.addtime(self.buffs['Saber Dance'].activation)
                    ...
                    elif self.abilities['Saber Dance'].available(nextgcd-1) and round(nextgcd - 1,2) >= self.clock and self.abilities['Technical Step'].nextuse <= nextgcd: # Wait for Second oGCD window for Saber dance
                        abilityused = False
                        saberhold = True
            ...
            if (oldpot != self.potency) or (oldesprit != self.esprit) or (oldfeathers != self.feathers):
                if self.createlog:
                    logging.info(str(self.clock) + ' : Potency: ' + str(round(self.potency, 1)) + ' || Feathers: ' + str(
